...
Pixmap fond;	/* pixmap avec juste le fond */
...
int initAffichage(int L, int H)
{
	...
	pix = XCreatePixmap (display, RootWindow(display,screen), L,H, prof);

	/* crée le pixmap de fond (dans lequel on rajoutera les obstacles ) */
	fond = XCreatePixmap (display, RootWindow(display,screen), L,H, prof);
	XSetForeground( display, gc, BlackPixel(display,screen) );
	XFillRectangle( display,fond,gc, 0,0, L,H);
	for(i=0;i<=L*H/500;i++)
	{
		XSetForeground( display, gc, gris[ (rand()&0x7000)>>12 ] );
		x=((rand()&0xFFF00)>>8)%L;
		y=((rand()&0xFFF00)>>8)%H;
		XFillRectangle( display,fond,gc, x,y,2,2);
	}

	/* copie du fond sur pix */
	XCopyArea( display, fond, pix, gc, 0,0, L,H, 0,0);
...
int miseAJourAffichage( int tempo_affichage)
{
	...
	XCopyArea( display, fond, pix, gc, 0,0, Largeur,Hauteur, 0,0);
